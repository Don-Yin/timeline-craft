...
function DroppableTagContainer({ id, isEmpty, isOver, children }: { id: string; isEmpty: boolean; isOver: boolean; children: React.ReactNode }) {
  ...
  return (
    <div
      ref={setNodeRef}
      className={`grid grid-cols-3 gap-2 items-start rounded-lg border-2 border-dashed p-2 min-h-[80px] transition-all duration-200 box-border w-full ${
...
function TagAccordion({ tag, slidesInTag, isOver, containerId, activeId, isExpanded, onToggle }: {
  ...
  return (
    <div className="rounded-lg border border-zinc-200 dark:border-zinc-700 overflow-hidden w-full box-border">
      <button
        onClick={onToggle}
      ...
      >
        <div className="flex items-center gap-2 min-w-0 flex-1">
          ...
          <span className="font-medium text-zinc-800 dark:text-zinc-200 truncate text-sm">{tag.label}</span>
        </div>
        <span className={`rounded-full px-2 py-0.5 text-xs font-medium transition-colors flex-shrink-0 ml-2 ${
...
export function TagSlideManager({ slides, tags, slideTagMap, onSlideMove }: TagSlideManagerProps) {
  ...
  return (
    <div className="flex flex-col h-full max-h-[calc(100vh-280px)] overflow-hidden w-full">
      <div className="flex items-center justify-between mb-3 px-1 flex-shrink-0">
        <h3 className="text-sm font-medium text-zinc-600 dark:text-zinc-400 truncate">arrange slides by section</h3>
      ...
      >
        <div className="flex-1 overflow-y-auto overflow-x-hidden space-y-2 pb-4 pr-1">
          {tags.map((tag) => {
