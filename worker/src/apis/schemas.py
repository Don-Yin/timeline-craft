...
class ProcessRequest(BaseModel):
    ...
    sidebar_width: float = Field(
        default=0.12, ge=0.05, le=0.5, description="Sidebar width as fraction of slide width (0.05-0.5)"
    )
    sidebar_item_height: float = Field(
        default=0.10, ge=0.03, le=0.3, description="Height of each tag item as fraction of slide height"
    )
    sidebar_init_font_size: int = Field(default=13, ge=8, le=24, description="Font size for sidebar text in points")
    sidebar_color_hex: str = Field(
        default="#6366f1", pattern=_HEX_COLOR, description="Sidebar fill color as hex (e.g., #6366f1)"
    )
    indicator_color_hex: str = Field(
        default="#111111", pattern=_HEX_COLOR, description="Indicator fill color as hex (e.g., #111111)"
    )
    ...
    sidebar_transparency: int = Field(
        default=95, ge=0, le=100, description="Sidebar transparency as percentage (0=opaque, 100=fully transparent)"
    )
    ...
    vertically_center: bool = Field(default=True, description="Whether to vertically center tags in the sidebar")
    rounded_indicator: bool = Field(default=True, description="Whether the indicator box has rounded corners")
    center_text: bool = Field(default=False, description="Whether to center text within the indicator box")
    compact_indicator: bool = Field(default=True, description="Whether indicator is narrower than sidebar width")
